...
def main():
    ...
    for id_ in LIVE_IDS:
        ...
        # Cartella + index.html, non più {id}.html — niente estensione
        # .html nell'URL pubblico, quindi niente scontro con la funzione
        # automatica di Cloudflare Pages che rimanda con 308 ogni richiesta
        # a un file .html verso la versione senza estensione (era la causa
        # del loop di redirect infinito su ogni pagina itinerario).
        out_dir = os.path.join(OUT_DIR, id_)
        os.makedirs(out_dir, exist_ok=True)
        out_path = os.path.join(out_dir, 'index.html')
        with open(out_path, 'w', encoding='utf-8') as f:
            f.write(page)
        print(f'  OK {id_}/index.html ({len(page.encode("utf-8"))} byte)')
